    ...
    <div>
      ...
      <div v-if="article" class="bg-white rounded-lg shadow overflow-hidden mb-6">
        ...
        <div class="h-48 sm:h-64 bg-gray-200 relative">
          ...
          <!-- Bookmark button (absolute position) -->
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          >
            ...
            <div>
              ...
              <p class="text-xs text-gray-500 mt-1">{{ formatDate(relatedArticle.publishedAt || relatedArticle.date) }}</p>
              <NuxtLink 
            ...
            <!-- Small bookmark icon for related articles -->
            <div class="absolute top-2 right-2">
              <button 
                @click.prevent.stop="toggleRelatedArticleBookmark(relatedArticle)"
                class="p-1 rounded-full hover:bg-gray-100 transition-colors"
                :class="{ 'text-blue-600': isBlogBookmarked(relatedArticle.id), 'text-gray-400': !isBlogBookmarked(relatedArticle.id) }"
                :title="isBlogBookmarked(relatedArticle.id) ? 'Hapus bookmark' : 'Tambahkan bookmark'"
              >
                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                  <path v-if="isBlogBookmarked(relatedArticle.id)" 
                    fill-rule="evenodd" 
...
// Toggle bookmark for related articles
const toggleRelatedArticleBookmark = async (relatedArticle) => {
    const isBookmarked = isBlogBookmarked(relatedArticle.id);
    
    if (isBookmarked) {
        await bookmarkService.removeBookmark(relatedArticle.id);
        bookmarkMessage.value = 'Artikel berhasil dihapus dari bookmark';
    } else {
        await bookmarkService.addBookmark({
            type: 'blog',
            blogId: relatedArticle.id,
            blogTitle: relatedArticle.title,
            blogExcerpt: relatedArticle.excerpt || truncateText(relatedArticle.content, 100),
            blogImage: relatedArticle.image
        });
        bookmarkMessage.value = 'Artikel berhasil ditambahkan ke bookmark';
    }
